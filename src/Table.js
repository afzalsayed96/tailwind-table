  ...
  return (
    <div className="min-w-full shadow overflow-hidden sm:rounded-lg border-gray-200">
      <div className="grid grid-cols-1 overflow-auto bg-white relative">
        <table className="table-fixed min-w-full divide-y divide-gray-200 col-start-1 col-span-1 row-span-1 row-start-1">
          <thead>
            <tr>
              <th className="px-6 py-3 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider invisible">
                {Object.keys(props.consolidated_holdings[0])[0]}
              </th>
              <th className="px-6 py-3 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider md:invisible">
                {Object.keys(props.consolidated_holdings[0])[1]}
              </th>
              <th className="px-6 py-3 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider lg:invisible">
              ...
              {Object.keys(props.consolidated_holdings[0])
                ...
                .map((column, index) => (
                  <th
                    className="px-6 py-3 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider"
                    key={index}
                  >
                    {column}
                  </th>
        ...
        <table className="table-fixed min-w-full divide-y divide-gray-200 sticky left-0 top-0 col-start-1 col-span-1 row-span-1 row-start-1">
          <thead>
            <tr>
              ...
              <th className="px-6 py-3 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider invisible md:visible md:bg-white border-r-2 border-gray-200 lg:border-0">
                {Object.keys(props.consolidated_holdings[0])[1]}
              </th>
              <th className="px-6 py-3 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider invisible lg:visible lg:bg-white border-r-2 border-gray-200">
                {Object.keys(props.consolidated_holdings[0])[2]}
              </th>
              {Object.keys(props.consolidated_holdings[0])
                .slice(3, -1)
          ...
          <tbody className="divide-y divide-gray-200">
            {props.consolidated_holdings.map((row, index) => (
              <tr key={index}>
                <td
                  className={
                    "whitespace-no-wrap text-sm leading-5 font-medium text-gray-900 px-6 py-3 bg-white border-r-2 border-gray-200 md:border-0" +
                    (typeof row[
                      Object.keys(props.consolidated_holdings[0])[0]
                    ] === "number"
